...
//
void scrape(const Stocks &stocks) {
    //
...
//            if (itest++ == ntest) return;
            ...
            int mxpno{0};
            ezCurl ez("ScrapeNasd", true);
            string doc_file = ez.GetResponseFile(nasdloc);
            //
            // parse the file
            ...
            for (int ip = mxpno; ip >= 1; --ip) {
                ...
                string flnm = "ScrapeNasd" + std::to_string(ic++);
                ezCurl c(flnm, false);
                c.init();
                FILE *F = c.setup(ploc);
                // using CURLMap = std::map<CURL*, tuple<URL, FILE*, DocFile>>;
                curls.emplace(c.get_curl(), std::make_tuple(c.get_URL(), F, ""));
                curl_multi_add_handle(curlm, c.get_curl());
            }
            ResVec responses;
...
//                                urls.erase(fnd);
                            ...
                            // using CURLMap = std::map<CURL*, tuple<URL, FILE*, DocFile>>;
                            auto f = std::get<1>(curls.find(e)->second);
                            if (f) fclose(f);
                            curl_multi_remove_handle(curlm, e);
                            auto d = std::get<2>(curls.find(e)->second);
                            responses.emplace_back(url, d);
                            continue;
                        }
                    } while (m);
                }
